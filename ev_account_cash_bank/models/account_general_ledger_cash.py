...
class AccountGeneralLedgerCashReport(models.AbstractModel):
    _inherit = "account.general.ledger"
    _name = 'account.general.funds'
    _description = 'General Funds'
    ...
    def _get_templates(self):
        templates = super(AccountGeneralLedgerCashReport, self)._get_templates()
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            if not any([display_name_part.startswith('111') for display_name_part in display_name.split(' ')]):
                continue
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(AccountGeneralLedgerCashReport, self)._get_options_domain(options)
