...
class AccountGeneralLedgerTransferReport(models.AbstractModel):
    _inherit = "account.general.ledger"
    _name = 'account.general.transfer'
    _description = 'General Transfer'
    ...
    def _get_templates(self):
        templates = super(AccountGeneralLedgerTransferReport, self)._get_templates()
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            if not any([display_name_part.startswith('113') for display_name_part in display_name.split(' ')]):
                continue
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(AccountGeneralLedgerTransferReport, self)._get_options_domain(options)
