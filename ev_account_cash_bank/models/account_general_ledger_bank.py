...
class AccountGeneralLedgerBankReport(models.AbstractModel):
    _inherit = "account.general.ledger"
    _name = 'account.general.banks'
    _description = 'General Banks'
    ...
    def _get_templates(self):
        templates = super(AccountGeneralLedgerBankReport, self)._get_templates()
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            if not any([display_name_part.startswith('112') for display_name_part in display_name.split(' ')]):
                continue
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(AccountGeneralLedgerBankReport, self)._get_options_domain(options)
